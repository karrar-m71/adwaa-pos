...
export default function SupplierDebts({user}){
  ...
  useEffect(()=>{ const u=onSnapshot(collection(db,'pos_suppliers'),s=>setList(s.docs.map(d=>({...d.data(),id:d.id})))); return()=>u(); },[]);
  const enrich=(p)=>{
  ...
  const print=()=>{
    ...
    d.setFont('helvetica','bold');d.setFontSize(14);d.text('Adwaa Al-Madina — Supplier Debts',105,15,{align:'center'});
    ...
    d.line(14,y,196,y);y+=5;d.setFont('helvetica','bold');d.text(`Total: ${totalIQD.toLocaleString()} IQD + ${totalUSD.toFixed(2)} USD`,14,y);d.save('SupplierDebts.pdf');
  };
  return(
    <div style={{padding:24,fontFamily:"'Cairo'",direction:'rtl'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        <div><div style={{color:'#fff',fontSize:22,fontWeight:800}}>🏭 ديون الموردين</div><div style={{color:'#64748b',fontSize:13}}>{sorted.length} مورد مدين — {fmt(totalIQD)} + {fmtUsd(totalUSD)}</div></div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:16}}>
        {[['🏭','عدد المدينين',sorted.length,'#f59e0b'],['💰','إجمالي الديون (IQD)',fmt(totalIQD),'#f59e0b'],['💵','إجمالي الديون (USD)',fmtUsd(totalUSD),'#3b82f6'],['✅','بدون ديون',list.filter(p=>!toNum(p.debtByCurrency?.IQD??p.debt??0)&&!toNum(p.debtByCurrency?.USD)).length,'#10b981']].map(([icon,label,val,color])=>(
      ...
      <div style={{background:'#ffffff',borderRadius:16,border:'1px solid #d9e2f2',overflow:'hidden'}}>
        ...
        {sorted.length===0?<div style={{color:'#10b981',textAlign:'center',padding:60,fontSize:15}}>✅ لا توجد ديون على الموردين!</div>
          :sorted.map((p,i)=>(
          <div key={p.id} style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr 1fr',padding:'11px 20px',borderBottom:i<sorted.length-1?'1px solid #ffffff':'none',alignItems:'center',background:i%2?'#f8fbff':'transparent'}}>
            ...
            <div style={{color:'#f59e0b',fontSize:14,fontWeight:800}}>{fmt(p.debtIQD)}</div>
            <div style={{color:'#3b82f6',fontSize:14,fontWeight:800}}>{fmtUsd(p.debtUSD)}</div>
            <div>
              <div style={{height:6,background:'#d9e2f2',borderRadius:3,overflow:'hidden',marginBottom:2}}>
                <div style={{width:`${Math.min(100,(p.debtEqIQD/(sorted.reduce((s,x)=>s+(x.debtEqIQD||0),0)||1))*100)}%`,height:'100%',background:'#f59e0b',borderRadius:3}}/>
              </div>
